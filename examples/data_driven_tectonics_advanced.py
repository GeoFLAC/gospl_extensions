...
2. Comparing different interpolation parameters for velocity data
3. Elevation field interpolation and sampling at external points
4. Time-dependent velocity fields with evolving parameters
5. Monitoring the effects of applied velocities over time
"""
...
def run_time_dependent_simulation(model, duration=10.0, dt=1.0):
    ...
    while current_time < duration:
        ...
        if hasattr(model, 'hdisp'):
            ...
            print(
                f"  Velocity stats - Max: {max_vel:.6f}, Mean: {mean_vel:.6f}")
...
def main():
    ...
    try:
        ...
        # Bind the DataDrivenTectonics methods
        ...
        # Example 3: Elevation interpolation
        print("\nExample 3: Elevation field interpolation")

        # Create a grid of points for elevation sampling
        x_vals = np.linspace(0, 10, 5)
        y_vals = np.linspace(0, 10, 5)
        xx, yy = np.meshgrid(x_vals, y_vals)
        sample_points = np.column_stack([xx.ravel(), yy.ravel(), np.zeros(25)])

        # Interpolate elevation field to these points
        elevations = model.interpolate_elevation_to_points(
            sample_points, k=5, power=1.0)
        print(f"Sampled elevations at {len(sample_points)} points:")
        print(f"  Min elevation: {np.min(elevations):.3f}")
        print(f"  Max elevation: {np.max(elevations):.3f}")
        print(f"  Mean elevation: {np.mean(elevations):.3f}")

        # Test different interpolation parameters for elevation
        print("\nComparing elevation interpolation parameters:")
        for k in [1, 3, 5]:
            elev_k = model.interpolate_elevation_to_points(
                sample_points, k=k, power=1.0)
            print(f"  k={k}: mean elevation = {np.mean(elev_k):.3f}")

        for power in [0.5, 1.0, 2.0]:
            elev_p = model.interpolate_elevation_to_points(
                sample_points, k=3, power=power)
            print(f"  power={power}: mean elevation = {np.mean(elev_p):.3f}")

        # Example 4: Time-dependent simulation
        print("\nExample 4: Time-dependent velocity simulation")
